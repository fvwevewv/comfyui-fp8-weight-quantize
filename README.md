...
为 ComfyUI 开发的权重量化与混合精度加载节点。提供基于原生的量化(comfy.quant_ops与comfy_kitchen)、基于 BitsAndBytes 的量化,以及使用随机舍入（Stochastic Rounding）,scale,Hadamard旋转(int8)方法旨在降低模型推理时的显存开销并优化访存效率。
...
* **技术原理（哈达玛旋转）**：
  - **Hadamard 旋转保护**：在将权重量化为 INT8 之前，通过对权重施加哈达玛矩阵正交变换，将通道中集中的激活值极值（Outliers）均匀分布到各个维度，从而在数学上缓解 W8A16 量化固有的数值溢出及偏色、底噪问题。
...
### SDXL (UNet 架构)
- **精度模式**：`float8_e4m3fn`
- **推理速度**：从 **`1.48 it/s`** 提升至 **`1.99 it/s`**（推理性能提升约 **34.5%**）
- **推理显存 (VRAM)**：**`5.3 GB`**

### Anima (DiT 架构)
- **精度模式**：`float8_e4m3fn`
- **推理速度**：从 **`2.0 s/it`** 缩短至 **`1.68 s/it`**（推理耗时减少约 **16%**）
- **推理显存 (VRAM)**：**`5.2 GB`**
---
